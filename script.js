...
const addNewDoc = () => {
    ...
    for (let j = 0; j < docNameUneditables.length; j++) {
        const stripDocNameUneditable = ((_a = docNameUneditables[j].textContent) === null || _a === void 0 ? void 0 : _a.split(".")[0]) || '';
...
const delBtn = document.querySelector("#del-btn"), delPathElement = delBtn.querySelector("svg path"), delPromptBox = document.querySelector("#delete-prompt-box-bg"), cfmDelBtn = document.querySelector("#cfm-n-del-btn"), docToDel = document.querySelector('#doc-to-del');
const mainInputSection = document.querySelector('.markdown-main-container'), markdownInputSection = document.querySelector('#markdown-input-section'), previewToggleDiv = document.querySelector("#preview-toggle"), previewToggle = previewToggleDiv.querySelector("span");
const sideBarToggleDiv = document.querySelector("#menu-close-toggle"), sideBarToggle = sideBarToggleDiv.querySelector("span");
const delDoc = () => {
    var _a;
    delPromptBox.style.display = "none";
    const allNewDocs = document.querySelectorAll('.new-doc');
    for (const div of allNewDocs) {
        if (div.innerHTML.includes(((_a = docToDel.textContent) === null || _a === void 0 ? void 0 : _a.replace(/[']+/g, '')) || '')) {
            div.remove();
        }
    }
